...
x86MoveImmediate tp dest_reg imm =
  x86MakeMovInstr tp dest_reg (D.qwordFromInteger imm)
...
x86StoreImmediate tp addr imm =
  x86MakeMovInstr tp (x86StackAddress addr tp) (D.qwordFromInteger imm)
...
x86OpSPImmediate op_bytes imm = do
  ...
  map noAddr $
    [makeInstr "mov" [tmp_reg, D.qwordFromInteger imm]]
    ++ (map fromFlexInst $
